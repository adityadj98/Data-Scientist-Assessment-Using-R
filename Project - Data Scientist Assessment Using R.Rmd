...
1. *Load library* dan *load dataset* dengan nama "trees_df". Adapun *library* yang diperlukan yaitu `readr`.

Begini :
```{r}
library(readr)

trees_df <- read_csv("https://storage.googleapis.com/dqlab-dataset/trees.csv", show_col_types = FALSE)
trees_df
```
...
trees_df$diameter.ft <- trees_df$Diameter*0.08333 #konversi inch ke ft
...
shapiro.test(trees_df$diameter.ft)
```
...
lm(formula = Volume ~ Height + diameter.ft, data = trees_df) #perhatikan posisi kolom dalam dataset di rumus Regresi nya
#melihat hubungan antara diameter.ft terhadap volume 
plot(trees_df$diameter.ft, trees_df$Volume)
